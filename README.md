...
- `skip_if_any_artist_male=true`：检查主歌手和 feat 里的所有 artist。
- `skip_if_any_artist_male=false`：只看第一个 main artist。
- `skip_only_when_all_artists_male=true`：只有被检查的非豁免 artist 全部是 male 时才跳过；男歌手和女歌手合作时会保留。
- `skip_only_when_all_artists_male=false`：恢复旧行为，被检查 artist 中任意一个是 male 就跳过。
...
输入 `male` 或 `m` 会立刻写入缓存，并且当前歌曲会继续按 male 规则处理；如果这首歌在当前配置下满足跳过条件且 `dry_run=false`，当前歌曲会被跳过。等待输入时如果 Spotify 已经播放到另一首歌，提示会自动取消，脚本会在下一轮处理新歌。
